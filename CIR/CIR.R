#' @title `CIR` Confidence Intervals of Ratios of content validity coefficients (confidence interval for diference of ratios)
#' @description Calculates confidence interval for the ratio of two content validity coefficients, based on method of
#' variance recovery for ratios (MOVER-R).
...
#' 
#' @return 
#' dataframe with four columns: label of the items, ratio between the coefficients, the upper and upper limit of the
...
#' 
#' If the two dataframes have different numbers of evaluated items (i.e., different numbers of rows), 'CIR' function 
#' matches the commonly labeled items, assuming they are the same items. 
#' 
#' Note: The function has not yet been prepared to resolve missing values, so the user must remove or impute any missing 
#' values.
#' 
#' @references
#' 
#' Zou, G., Donner, A. & Qiu, S. (2025). MOVER-R for Confidence Intervals of Ratios. In  N. Balakrishnan, T. Colton, 
#' B. Everitt, W. Piegorsch, F. Ruggeri and J.L. Teugels (Eds.), Wiley StatsRef: Statistics Reference Online. 
#' https://doi.org/10.1002/9781118445112.stat08085
#' 
#' Zou, G.Y. and Donner, A. (2008). Construction of confidence limits about effect measures: A general approach. 
#' Statist. Med., 27: 1693-1702. https://doi.org/10.1002/sim.3095
#' 
#' @seealso
...
CIR <- function(group1, group2, coef.col, lwr.col, upr.col) {
  ...
  # Filtrar los data.frames por ?tems comunes
  ...
  # Calcular R y los intervalos de confianza para cada ?tem
